          ...
          <div class="card-title d-flex align-items-center flex-wrap">
            ...
            <div>
              <div class="card-title d-flex align-items-center">
                <h1>{{ facility.building_name }}</h1>
              </div>
              <router-link :to="'/organizations/'+facility.organization_id" target="_blank"><h4 class="card-subtitle text-muted mb-2">{{ facility.organizations[0].organization_primary_name }} {{ facility.organizations[0].organization_primary_initial ? '(' + facility.organizations[0].organization_primary_initial + ')' : '' }}</h4></router-link>
            </div>
          </div>
      ...
      <b-card class="my-2" v-if="loaded">
        <b-card-body id="nav-scroller" ref="content" class="scrollbox">
          ...
          <div class="d-flex justify-content-between mb-3">
            ...
            <b-button v-show="!edit" variant='light' :href="'https://www.google.com/maps/search/?api=1&query='+formatLocationURLQuery(facility)" target="_blank"><b-icon icon="pin-map"></b-icon></b-button>
          </div>
          <div>
